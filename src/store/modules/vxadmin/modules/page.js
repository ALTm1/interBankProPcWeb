...
export default {
  ...
  actions: {
    /**
         * @description 确认已经加载多标签页数据 issues/201
         */
    isLoaded ({ state }) {
    ...
    openedLoad ({ state, commit, dispatch }) {
      return new Promise(async resolve => {
        console.dir(setting.page.opened)
        // store 赋值
        const value = await dispatch('vxadmin/db/get', {
          dbName: 'sys',
          path: 'page.opened',
    ...
    /**
         ...
         */
    open ({ state, commit, dispatch }, { name, params, query, path }) {
      return new Promise(async resolve => {
        // 已经打开的页面
        ...
        const pageOpend = opened.find((page, index) => {
          // let _path = page.fullPath ? page.fullPath.split('?')[0] : page.fullPath
          // console.log(page, _path, path)
          const same = page.fullPath === path
    ...
    close ({ state, commit, dispatch }, { tagName }) {
      return new Promise(async resolve => {
        ...
        // 若关闭的为最后一个，则跳转到首页
        if (isLast) {
          newPage = { name: 'index', params: {}, query: {} }
        }
    ...
    closeLeft ({ state, commit, dispatch }, { pageSelect } = {}) {
      return new Promise(async resolve => {
        ...
        if (currentIndex > 0) {
          // 删除打开的页面 并在缓存设置中删除
          state.opened.splice(0, currentIndex).forEach(({ name }) => commit('keepAliveRemove', name))
        }
