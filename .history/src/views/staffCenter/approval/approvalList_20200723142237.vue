      ...
      <ui-table header-row-class-name="header" :data="tableData" border>
        ...
        <ui-table-column fixed="right" label="操作" width="100">
          <template slot-scope="scope">
            <ui-button @click="handleClick(scope.row)" type="text" size="small" v-if="scope.row.state=='待审批'">审批</ui-button>
            <ui-button @click="toDetail(scope.row)" type="text" size="small" v-if="scope.row.state=='已审批'">详情</ui-button>
          </template>
        </ui-table-column>
      </ui-table>
    </div>
